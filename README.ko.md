...
매번 Elementor의 PHP 소스를 읽거나(비싸고, 그래도 JSON 형태는 알려주지
않습니다), 추측하거나(조용히 틀립니다). 이 스킬이 세 번째 선택지입니다:
...
대상으로 실행되며, 열화된 데이터의 출력을 거부하는 canary 3개가 지킵니다.
...
조회는 이렇게 합니다 — 질의 한 번에 약 700토큰이 들고, 답은 완결됩니다:
...
- `validate-page.py`는 Elementor가 잡아 주지 않는 것을 잡습니다: 알 수 없는
  ...
  `custom_messages` 없이 `success_message`를 설정하면 조용히 무시되는 대신
  경고가 납니다), multi-select 값, class 컨트롤에 들어간 숫자 `0`, Free 대상
  사이트의 Pro 전용 컨트롤, 그리고 **대상 사이트에 아예 존재할 수 없는
  위젯**까지.
...
- `verify-live.py`는 공개 URL을 캐시/CDN을 거쳐 가져와서, 트리와 CSS 값과
  wrapper class를 실제로 회선을 타고 내려온 것과 대조합니다.
...
`validate-page.py`는 대상 사이트가 가질 수 없는 위젯에 error를 냅니다. 사이트가
...
밀면 WooCommerce입니다. WooCommerce는 위젯 29개만 추가하는 것이 아니라 Pro
...
주입하며, 이들을 로드한 채 추출한 스키마는 그들의 컨트롤을 Elementor의 것인 양
실어 나릅니다.
...
믿지 말고 시험하십시오. 여덟 개의 검사가 여덟 개의 서로 다른 질문을, 서로 다른
산출물에서 읽어냅니다: 컨트롤 스택, 컴파일된 stylesheet, 전달된 HTML, 실제
...
9개가 실제로는 내보내지 않는 responsive breakpoint를 광고하고 있고, 스키마는
이제 그들에 대해 `rwd-BROKEN`이라고 말합니다.

**3. class를 내보내는 모든 컨트롤이 wrapper에 그 class를 붙이는가?**

2,573개 컨트롤은 CSS를 내보내는 대신 wrapper class를 덧붙이는 방식으로
동작합니다 — stylesheet 검사는 이들 전부에 대해 구조적으로 눈이 멉니다. 그래서
전달된 HTML에서 읽어냅니다: 98.2% 스윕, 실패 0. `classes_dictionary`의 legacy
remap(`position: "top"`이 `elementor-position-block-start`로 렌더링됨)과 장치별
prefix(`_tablet` suffix가 아니라 `elementor-tablet-position-`)까지 포함해서입니다.
...
아래 각각을 headless로 만든 뒤, 라이브 사이트의 브라우저에서 검증했습니다:
...
**8. PUBLIC이 받는 페이지에 이 전부가 들어 있는가?**
...
파일에 나뉘어 있습니다 — Kit의 globals는 다른 파일에 삽니다)를 edge cache를
거쳐 가져와서, 트리 + CSS 값 + wrapper class를 assert합니다. 변조된 트리에서는
실패합니다 — 한 번도 빨간불이 들어와 본 적 없는 verifier는 verifier가
아닙니다.

demo 페이지는 실재하고, 게시되어 있으며, 에디터에서 열린 적이 한 번도
...
4. 컨트롤은 세 가지 방식으로 gate됩니다. 컨트롤 499개는 빈 interpolated 값에서
   죽습니다
5. responsive 의존성은 breakpoint에서 다시 검사됩니다
6. `is_responsive`는 과잉 약속합니다 — 렌더링만이 압니다
7. CSS는 컨트롤이 할 수 있는 일의 절반일 뿐입니다 — 2,573개는 대신 class를
   내보냅니다
...
- **컨텍스트 의존 위젯**(cart, checkout, post comments, product 부품)은 빈
...
- **버전에 묶여 있음**: 여기 있는 모든 숫자는 Elementor 4.1.4 / Pro 4.1.2에서
  ...
  verifier가 함께 배포되고, *당신의* install에 대해 다시 돌아갑니다.
...
위젯을 통해, Pro 없이 블록을 페이지에 중첩하는 것까지 포함해서입니다.

## 기여

더 새로운 Elementor에 대해 다시 추출하고, 재생성한 `data/`로 PR을 여십시오 —
