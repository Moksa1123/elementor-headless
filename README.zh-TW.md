...
把 Elementor 完整的頁面編寫表面做成可查詢的資料庫，交給 AI coding agent——並且
...
所以每個 widget 都記錄著自己需要什麼，直接從其 module 在 Elementor 原始碼
...
讀入約快 5 倍，相較於載入 schema 則約快 118 倍。**工具延遲是實測的（每次查詢
中位數 316 ms）；讀入時間則是拿 token 數，用公開的 1,000 tok/s 參考速率
換算出來的——速率換掉，比值也不會變。自己重現一次；腳本會寫出
...
節省幅度曾經是 89.1%，後來反而**往下掉**了——因為這個表面變得誠實（WooCommerce、
...
它們一起抽出來的 schema，就會把這些 control 當成 Elementor 自家的東西
一併出貨。
...
CSS——樣式表檢查在結構上就注定一個也看不到。從送達的 HTML 讀出：99.8% 掃過、
...
hover control 對同一個屬性寫入刻意不同的值——必有一個輸），逐列分類。掃描
會先把 transition 關掉，而且報告裡明講有這道人為介入：transition 被刻意
seed 成 79 秒，開跑 200 ms 後讀到的顏色只是動畫中途的一格，做不得準。
...
抓取公開 URL 以及**該頁連結的每一份樣式表**（一頁的樣式分散在好幾個檔案
裡——Kit 的全域樣式放在另一個檔案），穿過 edge cache，並斷言樹、CSS 值與
...
用直覺的做法做上面任何一件事都會錯，錯法還可以一條一條編號，**整整十一種**——每一
...
- **Elementor V4 atomic elements**：僅支援查詢。建構它們是另一套資料模型，
  這個 skill 還不會寫。
- **依賴情境的 widget**（購物車、結帳、文章留言、商品頁的各個元件）在空白頁上驗證為
...
內容——包括不靠 Pro 也能把區塊一層層嵌進頁面，用的是免費的 shortcode widget。
