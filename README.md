...
**49,857 controls across 192 widgets and 13 elements, plus the Kit's 773 Site
Settings, 48 page settings, 29 document types, 51 dynamic tags, 39 display
conditions and every repeater's item fields** - as a queryable database instead
of a document it can never afford to read.
...
**86.8% fewer tokens than reading Elementor's source. 99.4% fewer than loading the
schema. ~5x faster on model ingest, ~118x vs loading the schema** (measured tool
latency: median 316 ms per query; ingest time derived from tokens at a disclosed
1,000 tok/s reference rate - change the rate, the ratio does not move). Reproduce
it - the script writes `data/token-benchmark.csv`:
...
The schema grew from 583k to 1.08M tokens as the surface got honest (WooCommerce,
the Kit, selectors, repeater fields), and the saving percentage moved DOWN from
89.1% - richer answers cost more tokens. Numbers that only ever improve are being
curated; these are regenerated by the script, whichever way they move.

Two things make it work: the data is **queried, never loaded**, and the 211
Advanced-tab controls that every widget shares are **stored once instead of 168
times** - they are the majority of all rows, so factoring them out keeps the
schema queryable at all.
...
**5. Every placeable widget, functionally, one per page.** The control sweeps ask
"does each setting work"; this asks the question before it - given content, does
the widget render it? Unique markers seeded into every content control (repeater
items built from the extracted fields), ONE widget per page so a JS error or a
zero-size render is attributable to exactly one widget, element screenshot each,
three viewports.
...
Per-widget rows in `data/widget-verification.csv`; one PNG per rendered widget in
`shots/`.

**6. Interactions, driven with a real pointer.** Rendering right is half of an
interactive widget. Five behaviours clicked for real on the public page:
...
actual pointer: hover the node the pseudo-class sits on, read `getComputedStyle`
on the node the declaration lands on, compare against the delivered stylesheet.
Transitions are disabled first, disclosed: reading a colour 200ms into a seeded
79-second transition compares a mid-animation frame against the end state.
...
**7. Verify the page the PUBLIC gets.** Everything above reads an artefact from
...
**8. Look at it.** `examples/demo-page.json` is a real published page, built with
...
tools/
  ...
  verify-browser.py              does a real Chromium COMPUTE it, on the right node?
